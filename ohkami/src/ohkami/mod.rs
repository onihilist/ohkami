...
impl Ohkami {
    ...
    async fn howl_core<T>(
        ...
        while let Some(accept) = ctrl_c.until_interrupt(__rt__::accept(&listener)).await {
            let Ok((connection, address)) = accept else {
                continue;
            };
            ...
            let connection: session::Connection = match &tls_acceptor {
                ...
                Some(tls_acceptor) => match ctrl_c
                    .until_interrupt(tls_acceptor.accept(connection))
                    .await
                {
                    None => break,
    ...
    pub fn __openapi_document_bytes__(&self, openapi: crate::openapi::OpenAPI) -> Vec<u8> {
        ...
        let mut bytes =
            ::serde_json::to_vec_pretty(&doc).expect("failed to serialize OpenAPI document");
        bytes.push(b'\n');
...
const _: () = {
    ...
    #[cfg(feature = "rt_lambda")]
    impl lambda_runtime::Service<lambda_runtime::LambdaEvent<crate::x_lambda::LambdaHTTPRequest>>
        for Ohkami
    {
        type Response = lambda_runtime::FunctionResponse<
            crate::x_lambda::LambdaResponse,
            std::pin::Pin<
                Box<dyn ohkami_lib::Stream<Item = Result<String, std::convert::Infallible>> + Send>,
            >,
        >;
...
#[cfg(feature = "__rt_native__")]
mod sync {
    pub struct WaitGroup(std::ptr::NonNull<std::sync::atomic::AtomicUsize>);
    const _: () = {
    ...
    const _: () = {
        ...
        static WAKERS: std::sync::RwLock<Vec<AtomicPtr<Waker>>> =
        ...
        __rt__::testing::block_on(async {
            ...
            let (cert_file_path, key_file_path) = {
                let target_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                    ...
                    .join("target");
                (
                    target_dir.join("test-cert.pem"),
                    target_dir.join("test-key.pem"),
                )
            };
            ...
            let key = rustls_pemfile::read_one(&mut std::io::BufReader::new(
                std::fs::File::open(key_file_path).unwrap(),
            ))
            .expect("Failed to read private key")
            .map(|p| match p {
